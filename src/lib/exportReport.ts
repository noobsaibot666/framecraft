...
export function generateDeliveryReceipt(report: ExportReport, deliveredAt: string): string {
  const p = report.project;
  const winners = report.prompts.filter((pr) => pr.is_winner);
  const shots = report.shots;

  const lines: string[] = [
    `# Delivery Receipt — ${p.title}`,
    "",
    `**Client:** ${p.client ?? "—"}  `,
    `**Delivered:** ${deliveredAt}  `,
    `**Provider:** ${[...new Set(winners.map((w) => w.provider))].join(", ") || "—"}  `,
    "",
    "---",
    "",
    `## Summary`,
    "",
    `- ${winners.length} winning prompt${winners.length !== 1 ? "s" : ""}`,
    `- ${shots.length} shot${shots.length !== 1 ? "s" : ""} in sequence`,
    `- ${shots.filter((s) => s.result_id).length} shots with confirmed results`,
    "",
    "---",
    "",
  ];
  ...
  if (winners.length > 0) {
    lines.push("## Delivered Prompts", "");
    for (const w of winners) {
      lines.push(`### ${w.title}`, "");
      lines.push("```");
      lines.push(w.prompt_text);
      lines.push("```", "");
    }
    lines.push("---", "");
  }
  ...
  if (shots.length > 0) {
    ...
    lines.push("|---|------|-------|--------|--------|");
    for (const s of shots) {
      const num = s.sort_order + 1;
      const prompt = s.prompt_title ?? "—";
      const result = s.result_is_winner ? "★ Winner" : s.result_score !== undefined ? `Score ${s.result_score}` : "—";
      lines.push(`| ${num} | ${s.shot_type} | ${s.label} | ${prompt} | ${result} |`);
    }
    lines.push("");
  }

  lines.push("---", "", "*Generated by Framecraft*");
  return lines.join("\n");
}
